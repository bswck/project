...
export REPO_URL
REPO_URL=$(cat <<- 'EOF'
	https://github.com/bswck/project
EOF
)
export COVERAGE_URL
COVERAGE_URL=$(cat <<- 'EOF'
	https://coverage-badge.samuelcolvin.workers.dev/redirect/bswck/project
EOF
)
export DOCS_URL
DOCS_URL=$(cat <<- 'EOF'
...
TIDELIFT_URL=$(cat <<- 'EOF'
	https://tidelift.com/subscription/pkg/pypi-project?utm_source=pypi-project
EOF
)
export SKELETON
SKELETON=$(cat <<- 'EOF'
...
GH_REPO_ARGS=$(cat <<- 'EOF'
	"bswck/project" --public --source=./ --remote=upstream --description="Set up projects for development quickly."
EOF
)
export GH_ENSURE_ENV
GH_ENSURE_ENV=$(cat <<- 'EOF'
	jq -n '{"deployment_branch_policy": {"protected_branches": false,"custom_branch_policies": true}}' | gh api -H "Accept: application/vnd.github+json" -X PUT "/repos/bswck/project/environments/$1" --silent --input -
EOF
)
